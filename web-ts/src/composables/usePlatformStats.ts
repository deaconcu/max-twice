...
interface FormattedPlatformStats {
  courseCount: string
  careerPathCount: string
  roadmapCount: string
  knowledgeNodeCount: string
  articleCount: string
  rawData: PlatformStats | null
}

// 全局缓存状态，在多个组件间共享
const globalStats: Ref<FormattedPlatformStats | null> = ref(null)
...
// 从localStorage获取缓存数据
const getCachedData = (): FormattedPlatformStats | null => {
  try {
...
// 缓存数据到localStorage
const setCachedData = (data: FormattedPlatformStats): void => {
  try {
    const cacheData = {
      data,
      timestamp: Date.now(),
...
// 获取平台统计数据
const fetchPlatformStats = async (force = false): Promise<FormattedPlatformStats | null> => {
  // 如果正在加载中，避免重复请求
  ...
  try {
    ...
    if (response.code === 200 && response.data) {
      ...
      // 格式化数据
      const formattedStats: FormattedPlatformStats = {
      ...
      console.warn('Platform stats API returned error:', response.message || '获取平台数据失败')
      setDefaultStats()
      return globalStats.value
    }
  } catch (err) {
