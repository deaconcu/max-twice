...
// 保存数据到localStorage
const saveLocalData = (data: LocalData): void => {
  try {
...
export const useStudyTimeTracker = () => {
  ...
  const currentDuration: Ref<number> = ref(0) // 当前会话累计时长（秒）
  const todayTotal: Ref<number> = ref(0) // 今日总学习时长（秒）
  ...
  // 更新每日统计
  const updateDailyStats = (additionalDuration: number): void => {
  ...
  // 计算连续学习天数
  const calculateContinuousStreak = (): number => {
    const today = getTodayString()
    ...
    const hasStudiedToday =
      localData.dailyStats[today] && localData.dailyStats[today].totalDuration > 0
    const hasStudiedYesterday =
      localData.dailyStats[yesterdayString] &&
      localData.dailyStats[yesterdayString].totalDuration > 0
  ...
  // 开始追踪学习时间
  const startTracking = (courseId: number, nodeId: number | null = null): void => {
    if (isTracking.value) {
    ...
    // 保存定时器引用以便清理
    if (!(window as any).studyTrackingTimers) {
      (window as any).studyTrackingTimers = []
    }
    ;(window as any).studyTrackingTimers.push(updateTimer)
  ...
  const stopTracking = (): void => {
    ...
    // 清理更新定时器
    if ((window as any).studyTrackingTimers) {
      ;(window as any).studyTrackingTimers.forEach((timer: NodeJS.Timeout) => clearInterval(timer))
      ;(window as any).studyTrackingTimers = []
    }
