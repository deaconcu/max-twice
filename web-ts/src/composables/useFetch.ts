...
export function useFetch<T>(options: FetchOptions<T>): FetchReturn<T> {
    ...
    try {
      ...
      if (result !== null) {
        ...
        if (transform && Array.isArray(result)) {
          data.value = result.map(transform as any) as T
        } else if (transform && !Array.isArray(result)) {
          data.value = transform(result as any) as T
        } else {
          data.value = result
        }
      }
  ...
  if (debounceDelay) {
    execute = debounce(executeCore, debounceDelay)
  } else if (throttleDelay) {
    execute = throttle(executeCore, throttleDelay)
  }
