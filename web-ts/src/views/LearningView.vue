<script setup lang="ts">
  ...
  const selectedRoadmapStatus = ref<string>('all') // 路线图状态筛选: all, USER_ROADMAP_STATE.NOT_STARTED, USER_ROADMAP_STATE.IN_PROGRESS, USER_ROADMAP_STATE.COMPLETED
  const selectedCourseStatus = ref<string>('all') // 课程状态筛选: all, USER_COURSE_STATE.NOT_STARTED, USER_COURSE_STATE.IN_PROGRESS, USER_COURSE_STATE.COMPLETED
  ...
  const selectedRoadmap = ref<ProcessedRoadmap | null>(null)
  ...
  const parseContent = (content: string | any, profession: Profession | null = null): { nodes: NodeBase[], edges: Edge[] } => {
    try {
      ...
      const nodes = (data.nodes || []).map((node: any, index: number): NodeBase => {
        ...
        return {
          ...
          data: {
            ...
            completed, // 适配新的finished字段
            progress, // 使用新的progress字段
      ...
      // console.error('解析课程内容失败:', content)
      return { nodes: [], edges: [] }
    }
  }

  // 筛选后的学习数据
  const filteredLearningData = computed((): LearningData => {
    const filtered = {
      ...learningData.value,
      roadmaps: learningData.value.roadmaps.filter((roadmap) => {
        // 路线图状态筛选
        if (
          selectedRoadmapStatus.value !== 'all' &&
          roadmap.state !== Number(selectedRoadmapStatus.value)
        ) {
          return false
        }
        // 搜索筛选
  ...
  const loadLearningData = async (): Promise<void> => {
    try {
      ...
      let roadmaps: ProcessedRoadmap[] = []
      let courses: ProcessedCourse[] = []

      // 处理路线图数据
      if (roadmapResponse.code === 200 && Array.isArray(roadmapResponse.data)) {
        roadmaps = (roadmapResponse.data as UserRoadmap[]).map((userRoadmap: UserRoadmap): ProcessedRoadmap => {
      ...
      if (courseResponse.code === 200 && Array.isArray(courseResponse.data)) {
        courses = (courseResponse.data as UserCourse[]).map((userCourse: UserCourse): ProcessedCourse => {
          return {
            id: userCourse.id,
            ...
            state: String(userCourse.state), // 转换为字符串
  ...
  // 计算总体进度
  const calculateOverallProgress = (roadmaps: ProcessedRoadmap[]): number => {
  ...
  const closeRoadmap = (roadmap: any, event?: Event): void => {
    ...
    //if (window.confirm(t('learning.confirmExit'))) {
    // 这里可以调用后端API来退出学习
  ...
  const closeCourse = (course: any, event?: Event): void => {
    ...
    // TODO: 实现关闭逻辑，例如显示确认对话框
    //if (window.confirm(t('learning.exitCourseConfirm'))) {
    //  这里可以调用后端API来退出学习
