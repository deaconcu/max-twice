<script setup lang="ts">
  ...
  const openProfession = (professionId: string | number): void => {
    const url = router.resolve({ path: '/roadmap', query: { professionId } }).href
    window.open(url, '_blank')
  }
  ...
  const handleItemClick = (item: RankingItem): void => {
    if (activeTab.value === 'courses') {
      openCourse(item.id)
    } else {
      ...
      <v-col cols="12" class="mb-4">
        <div class="d-flex align-center justify-space-between">
          <div class="d-flex align-center">
            ...
            ></v-btn>
            <div>
              <h1 class="text-h4 font-weight-bold text-grey-darken-4 mb-1">热门排行榜</h1>
        ...
        <v-card flat color="grey-lighten-5" rounded="xl" class="sticky-nav">
          <v-card-text class="pa-4">
            ...
            <div class="mt-6 pa-3 rounded-lg bg-white">
              <h4 class="text-body-1 font-weight-bold text-grey-darken-4 mb-2">📊 统计概览</h4>
                ...
                >
                  ...
                  >
                    ...
                    <div v-if="activeTab === 'courses'" class="d-flex justify-space-around">
                      <div class="text-center">
                        <div class="text-h6 font-weight-bold text-primary">
                          {{ ((item as Course).learnerCount || 0).toLocaleString() }}
                        </div>
                        <div class="text-caption text-grey-darken-1">学习</div>
                      </div>
                      <div class="text-center">
                        <div class="text-h6 font-weight-bold text-success">
                          {{ ((item as Course).subscriptionCount || 0).toLocaleString() }}
                        </div>
                    ...
                    <div v-else class="text-center">
                      <div class="text-h6 font-weight-bold text-primary">
                        {{ ((item as Profession).learnerCount || 0).toLocaleString() }}
                      </div>
                    ...
                    <div v-if="activeTab === 'courses'" class="d-flex align-center">
                      ...
                      <div class="text-center">
                        <div class="text-h6 font-weight-bold text-grey-darken-2">
                          {{
                            (
                              ((item as Course).learnerCount || 0) + ((item as Course).subscriptionCount || 0)
                            ).toLocaleString()
                          }}
                        </div>
                        <div class="text-caption text-grey-darken-1">
