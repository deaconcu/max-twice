<script setup lang="ts">
  ...
  type ModuleName = 'platform' | 'learning' | 'tips' | 'careers' | 'courses'
  ...
  const props = withDefaults(defineProps<Props>(), {
    enabledModules: () => ['platform', 'learning', 'tips', 'careers', 'courses'],
    excludeModules: () => [],
  })
  ...
  // 获取排名芯片颜色
  const getRankChipColor = (index: number): string => {
    switch (index) {
      case 0:
        return 'gold'
      case 1:
  ...
  // 跳转到职业详情
  const goToCareerDetail = (profession: Profession): void => {
    router.push(`/roadmap/${profession.id}`)
  }

  // 跳转到路线图详情
  const goToRoadmapDetail = (roadmap: LearningRoadmap): void => {
    router.push(`/roadmap/${roadmap.id}`)
  }

  // 跳转到课程详情(用户课程)
  const openCourseInNewTab = (course: LearningCourse): void => {
  ...
  // 跳转到课程详情
  const openInNewTab = (courseId: number): void => {
    const url = router.resolve({ path: '/read', query: { courseId } }).href
    window.open(url, '_blank')
  }
</script>
    ...
    >
      <v-card-text class="pa-4">
        ...
        >
          ...
          <div class="flex-grow-1">
            <div class="text-body-2 font-weight-bold text-grey-darken-4">{{ profession.name || t('rightSidebar.unknownProfession') }}</div>
            <div class="text-caption text-grey-darken-2">
              {{
