...
const localSelectedTab = computed({
  get: (): string => props.selectedTab,
  set: (value: string): void => emit('update:selectedTab', value),
})

const localSelectedNavTab = computed({
  get: (): string => props.selectedNavTab,
  set: (value: string): void => emit('update:selectedNavTab', value),
})

const localSelectedStatus = computed({
  get: (): string => props.selectedStatus,
  set: (value: string): void => emit('update:selectedStatus', value),
})

const localSearchQuery = computed({
  get: (): string => props.searchQuery,
  set: (value: string): void => emit('update:searchQuery', value),
})
          ...
          <div class="d-flex align-center">
            ...
            >
              <v-btn
                value="learning"
                class="nav-btn"
                :class="{ 'nav-btn-active': localSelectedNavTab === 'learning' }"
              >
                {{ t('learning.title') }}
              </v-btn>
    ...
    <v-row justify="start" align="center" class="mb-4">
      ...
      <v-col cols="5" class="d-flex justify-start">
        ...
        >
          ...
          <v-btn
            :value="currentStateConstants.NOT_STARTED"
            size="small"
            class="me-1 rounded-lg text-body-2"
          >
            <v-icon icon="mdi-circle-outline" class="mr-1" size="14"></v-icon>
            {{ getStateText(currentStateConstants.NOT_STARTED) }}
          </v-btn>
          <v-btn
            :value="currentStateConstants.IN_PROGRESS"
            size="small"
            class="me-1 rounded-lg text-body-2"
          >
          ...
          <v-btn
            :value="currentStateConstants.COMPLETED"
            size="small"
            class="me-1 rounded-lg text-body-2"
          >
