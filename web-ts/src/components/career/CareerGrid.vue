...
interface Props {
  ...
  currentCareers: CareerWithDisplay[]
  careers: CareerWithDisplay[]
...
interface Emits {
  (e: 'loadMoreCareers', loadOptions: { done: (status: string) => void }): void
  (e: 'goToCareerDetail', career: CareerWithDisplay): void
  (e: 'goBackToSecondLevel'): void
}
...
// 处理加载更多
const handleLoadMoreCareers = (loadOptions: { done: (status: string) => void }): void => {
...
// 处理返回二级分类
const handleGoBackToSecondLevel = (): void => {
...
// 判断是否显示职业列表
const showCareerList: ComputedRef<boolean> = computed(() => {
