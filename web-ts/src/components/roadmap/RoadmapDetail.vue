  ...
  <v-dialog v-model="visible" fullscreen :scrim="true" transition="dialog-bottom-transition">
    <v-card>
      <v-toolbar color="grey-lighten-5" flat class="flat-toolbar" density="compact">
        ...
        <v-spacer></v-spacer>
        <v-btn icon variant="text" size="small" class="flat-icon-button" @click="$emit('close')">
          <v-icon color="teal-darken-1">mdi-close</v-icon>
        </v-btn>
      </v-toolbar>

      <v-container v-if="roadmap" fluid class="pa-0 roadmap-container">
        <v-row no-gutters class="full-height">
          <!-- 左侧课程表区域 -->
          <v-col cols="8" class="roadmap-detail-content full-height">
            <v-card flat class="roadmap-card-container">
              <v-card-text
                  ...
                  >
                    {{
                      isDescriptionExpanded
                        ? t('roadmapDetail.collapse')
                        : t('roadmapDetail.viewMore')
                    }}
...
<script setup lang="ts">
  ...
  interface Emits {
    ...
    (e: 'close'): void
    (e: 'node-click', data: { node: Node }): void
  ...
  const shouldShowToggle = ref<boolean>(false)
  ...
  // 检查描述是否超过100px
  const checkDescriptionHeight = async (): Promise<void> => {
    if (!props.roadmap?.description) return

    await nextTick()
    const element = document.querySelector('.description-text') as HTMLElement
    if (element) {
