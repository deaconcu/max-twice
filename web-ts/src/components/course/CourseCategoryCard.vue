<script setup lang="ts">
  ...
  interface Emits {
    ...
    (e: 'selectSubCategory', categoryIndex: number, subIndex: number): void
    (e: 'openCourse', courseId: string | number): void
  }
  ...
  // 处理一级分类切换
  const handleToggleFirstLevel = (): void => {
    emit('toggleFirstLevel', props.categoryIndex)
  }

  // 处理二级分类选择
  const handleSelectSubCategory = (subIndex: number): void => {
    emit('selectSubCategory', props.categoryIndex, subIndex)
  }

  // 处理课程打开
  const handleOpenCourse = (courseId: string | number): void => {
