<script setup lang="ts">
  ...
  // 加载课程列表
  const loadCourses = async (isLoadMore: boolean = false): Promise<void> => {
    try {
      if (isLoadMore) {
      ...
      if (response.code === 200) {
        ...
        showSnackbar && showSnackbar('加载课程失败: ' + (response.msg || '未知错误'))
      }
    } catch (error) {
      console.error('Error loading courses:', error)
      showSnackbar && showSnackbar('加载课程失败: 服务器开小差了')
    } finally {
      loading.value = false
      loadingMore.value = false
    }
  ...
  const onLoadMore = async ({ done }: { done: InfiniteScrollCallback }): Promise<void> => {
    ...
    try {
      ...
      showSnackbar && showSnackbar('加载更多失败', 'error')
      done('error')
    }
  }
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        if (subcourseList.value.length > 0) {
          showSnackbar && showSnackbar(`查询成功，找到 ${subcourseList.value.length} 个子课程`, 'success')
        } else {
          showSnackbar && showSnackbar('查询成功', 'success')
        }
      } else {
        searchedCourse.value = null
        showSnackbar && showSnackbar('未找到该课程', 'warning')
      }
    } catch (error) {
      console.error('查询课程失败:', error)
      searchedCourse.value = null
      subcourseList.value = []
      showSnackbar && showSnackbar('查询失败，请重试', 'error')
    } finally {
    ...
    try {
      ...
      if (response.code === 200 && response.data) {
        ...
        if (categories.length > 0) {
          const firstMapping = mapping.find((m: CategoryMapping) => m.mainCategoryId === categories[0].id)
  ...
  const confirmEdit = async (): Promise<void> => {
    ...
    try {
      ...
      if (response.code === 200) {
        showSnackbar && showSnackbar('课程信息已更新')
        ...
        showSnackbar && showSnackbar('更新失败', 'error')
      }
    } catch (error) {
