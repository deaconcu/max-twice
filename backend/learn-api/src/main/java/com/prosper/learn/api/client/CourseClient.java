...
public interface CourseClient {
    ...
    Response<CourseDTOV4> get(@PathVariable int id);
    ...
    Response<Object> getListByState(@RequestParam String state, @RequestParam(value = "lastId", defaultValue = "0") int lastId);
    ...
    Response<Object> getApprovedListByParent(@RequestParam int parentId);
    ...
    Response post(@RequestParam String name, @RequestParam String description, @RequestParam int parentId);

    // 修改
    // 管理员批准，修改课程
    // 用户修改简介，名称
    @PutMapping("/course/{id}")
    Response put(@PathVariable int id, @RequestBody CourseDTO course);
}
