...
public class AutoAuthorGenerationService {
    ...
    private String buildPrompt(NodeDO nodeDO, CourseDO courseDO) {
        return """
                请忽略之前所有对话内容，独立回答
                我在写一本教材，教材名称是："%s", 我当前在 "%s" 这个目录下，" +
                请你判断：
                1. 如果能用一篇文章说清楚这个目录的内容，就生成一篇文章:
                   文章前用"[A]"开标注这是一篇文章，需要使用容易让人理解的方式，返回的内容要带markdown的格式，不要有和内容无关的语言，请根据实际情况调整长度，文章要非常的侧重清楚明白，容易理解;
                   示例：[A]文章内容
                2. 如果这个目录下的内容比较多，不能用一篇文章说清楚，就生成一个子目录列表:
                   目录前用"[C]"来标注这是一个目录，目录是一个一级目录，使用["...", "...", ...]的格式，前后不要附加任何文字，目录中不要添加章节序号，不要添加无关的内容，比如欢迎语，只有目录
                   示例：[C]["绪论", "基础知识", "进阶篇"]
               """.formatted(courseDO.getName(), nodeDO.getName());
    }
