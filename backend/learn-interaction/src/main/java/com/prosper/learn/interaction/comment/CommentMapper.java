...
public interface CommentMapper {
    ...
    @Select("SELECT * FROM comment where object_id = #{objectId} and object_type = #{objectType} and reply_to_comment_id = 0 and state = " + ContentState.PUBLISHED_VALUE + " ORDER BY score DESC, id DESC limit #{count}")
    List<CommentDO> getByObjectId(long objectId, int objectType, int count);

    // 分页加载评论，处理分数相同的情况，只显示已通过的评论
    @Select("SELECT * FROM comment where object_id = #{objectId} and object_type = #{objectType} and reply_to_comment_id = 0 and state = " + ContentState.PUBLISHED_VALUE + " AND " +
    ...
    @Select("SELECT * FROM comment where reply_to_comment_id = #{commentId} and state = " + ContentState.PUBLISHED_VALUE + " ORDER BY score DESC, id DESC limit #{count}")
    List<CommentDO> getByTopic(long commentId, int count);

    // 分页加载话题回复，处理分数相同的情况，只显示已通过的评论
    @Select("SELECT * FROM comment where reply_to_comment_id = #{commentId} and state = " + ContentState.PUBLISHED_VALUE + " AND " +
            "(score < #{lastScore} OR (score = #{lastScore} AND id < #{lastId})) " +
            "ORDER BY score DESC, id DESC limit #{count}")
