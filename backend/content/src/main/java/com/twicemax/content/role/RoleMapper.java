...
public interface RoleMapper {
    ...
    @Select("<script>" +
            "SELECT * FROM role WHERE main_category = #{mainCategory} AND state = " + Enums.ContentState.PUBLISHED_VALUE + " AND deleted_at IS NULL " +
            "<if test='lastId != null'>AND id &lt; #{lastId}</if> " +
            "ORDER BY id DESC LIMIT #{limit}" +
            "</script>")
    ...
    @Select("<script>" +
            "SELECT * FROM role WHERE sub_category = #{subCategory} AND state = " + Enums.ContentState.PUBLISHED_VALUE + " AND deleted_at IS NULL " +
            "<if test='lastId != null'>AND id &lt; #{lastId}</if> " +
            "ORDER BY id DESC LIMIT #{limit}" +
            "</script>")
    ...
    @Select("<script>" +
            "SELECT * FROM role WHERE main_category = #{mainCategory} AND sub_category = #{subCategory} AND state = " + Enums.ContentState.PUBLISHED_VALUE + " AND deleted_at IS NULL " +
            "<if test='lastId != null'>AND id &lt; #{lastId}</if> " +
            "ORDER BY id DESC LIMIT #{limit}" +
            "</script>")
    List<RoleDO> listByMainCategoryAndSubCategoryAndLastId(int mainCategory, int subCategory, Long lastId, int limit);

    @Select("SELECT * FROM role WHERE state = " + Enums.ContentState.PUBLISHED_VALUE + " AND deleted_at IS NULL AND name LIKE CONCAT('%', #{keyword}, '%') ORDER BY id DESC LIMIT 20")
    ...
    // 平台统计相关方法
    @Select("SELECT COUNT(*) FROM role WHERE state = " + Enums.ContentState.PUBLISHED_VALUE + " AND deleted_at IS NULL")
