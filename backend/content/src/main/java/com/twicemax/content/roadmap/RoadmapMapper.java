...
public interface RoadmapMapper {
    ...
    @Select({"<script>",
             ...
             "<if test='state == null'> AND r.state != " + Enums.ContentState.BANNED_VALUE + "</if>",
     ...
     * 根据角色获取路线图列表（按创建时间排序）
     */
    @Select("SELECT * FROM roadmap WHERE role_id = #{roleId} AND state = " + Enums.ContentState.PUBLISHED_VALUE + " AND deleted_at IS NULL " +
    ...
    // 平台统计相关方法
    @Select("SELECT COUNT(*) FROM roadmap WHERE state = " + Enums.ContentState.PUBLISHED_VALUE + " AND deleted_at IS NULL")
