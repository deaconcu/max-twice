...
public interface PostMapper {
    ...
    @Select({"<script>",
            ...
            "AND n.state = " + Enums.ContentState.PUBLISHED_VALUE,
            "AND c.state = " + Enums.ContentState.PUBLISHED_VALUE,
     ...
     * @return 文章总数
     */
    @Select("SELECT COUNT(*) FROM post WHERE state = " + Enums.ContentState.PUBLISHED_VALUE + " AND deleted_at IS NULL")
    Long countActiveArticles();

    @Select("SELECT COUNT(*) FROM post WHERE node_id = #{nodeId} AND creator_id = #{creatorId} AND state = " + Enums.ContentState.PUBLISHED_VALUE + " AND deleted_at IS NULL")
