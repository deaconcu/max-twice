...
public interface CourseMapper {
     ...
     */
    @Select("SELECT * FROM course WHERE name LIKE CONCAT('%', #{name}, '%') AND state = " + Enums.ContentState.PUBLISHED_VALUE + " LIMIT #{limit}")
    ...
    List<CourseDO> listByParentAndState(Enums.ContentState state, long parentCourseId);
    ...
    @Select("<script>" +
            ...
            "AND state = " + Enums.ContentState.PUBLISHED_VALUE + " AND parent_course_id = 0 " +
            "<if test='lastId != null'>AND id &lt; #{lastId}</if> " +
    ...
    // 新增：课程状态操作方法
    @Update("UPDATE course SET state = " + Enums.ContentState.PUBLISHED_VALUE + ", reason = '' WHERE id = #{id}")
    int approve(long id);

    @Update("UPDATE course SET state = " + Enums.ContentState.REJECTED_VALUE + ", reason = #{reason} WHERE id = #{id}")
    int reject(long id, String reason);

    @Update("UPDATE course SET state = " + Enums.ContentState.BANNED_VALUE + ", reason = #{reason} WHERE id = #{id}")
    ...
    // 平台统计相关方法
    @Select("SELECT COUNT(*) FROM course WHERE state = " + Enums.ContentState.PUBLISHED_VALUE)
     ...
     * 统计某个父课程的已发布子课程数量
     */
    @Select("SELECT COUNT(*) FROM course WHERE parent_course_id = #{parentCourseId} AND state = " + Enums.ContentState.PUBLISHED_VALUE)
