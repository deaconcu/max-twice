...
export function useUserPostsQuery(userId: MaybeRef<number>, type = 2, enabled?: MaybeRef<boolean>) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => userApi.getUserPosts(toValue(userId), pageParam as string | undefined, type),
...
export function useMyPostsQuery(type?: MaybeRef<number | undefined>, state?: MaybeRef<number | undefined>, enabled?: MaybeRef<boolean>) {
  return useInfiniteQuery({
    queryKey: computed(() => userKeys.myPosts(undefined, toValue(type), toValue(state))),
    queryFn: ({ pageParam }) =>
      userApi.getCurrentUserAllPosts(pageParam as string | undefined, toValue(type), toValue(state)),
    initialPageParam: undefined as string | undefined,
    getNextPageParam: (lastPage) => lastPage.hasMore ? lastPage.nextCursor ?? undefined : undefined,
    enabled,
  })
...
export function useUserRoadmapsQuery(userId: MaybeRef<number>, enabled?: MaybeRef<boolean>) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => userApi.getUserRoadmaps(toValue(userId), pageParam as string | undefined),
    initialPageParam: undefined as string | undefined,
    getNextPageParam: (last) =>
      Array.isArray(last) && last.length === 20 ? (last[last.length - 1] as { id: number }).id.toString() : undefined,
    enabled,
  })
}

export function useMyRoadmapsQuery(state?: MaybeRef<number | undefined>, enabled?: MaybeRef<boolean>) {
  return useInfiniteQuery({
    queryKey: computed(() => userKeys.myRoadmaps(undefined, toValue(state))),
    queryFn: ({ pageParam }) => userApi.getCurrentUserRoadmaps(pageParam as string | undefined, toValue(state)),
    initialPageParam: undefined as string | undefined,
    getNextPageParam: (last) =>
      Array.isArray(last) && last.length === 20 ? (last[last.length - 1] as { id: number }).id.toString() : undefined,
    enabled,
