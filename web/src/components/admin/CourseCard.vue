<template>
  <v-card
    flat
    class="course-card border rounded-lg pa-4 mb-3"
    hover
    elevation="0"
  >
    <v-row no-gutters>
      ...
      <v-col cols="12" md="2" class="pr-md-4 mb-3 mb-md-0">
        <div class="d-flex flex-column h-100">
          ...
          <div class="mb-3">
            <v-chip
              :color="getStateConfig(course.state).color"
              variant="flat"
              rounded="lg"
              size="small"
              class="font-weight-medium"
            >
              <v-icon :icon="getStateConfig(course.state).icon" size="14" class="mr-1"></v-icon>
              {{ getStateConfig(course.state).text }}
            </v-chip>
          </div>
          ...
          <div class="d-flex flex-column ga-2">
            ...
            <!-- 待审核状态：批准和拒绝 -->
            <template v-if="course.state === 'SUBMITTED'">
              <!-- 批准按钮 -->
              <v-btn
                variant="flat"
            ...
            <!-- 已通过状态：删除 -->
            <template v-if="course.state === 'APPROVED'">
              <!-- 拒绝按钮 -->
              <v-btn
                variant="flat"
            ...
            <!-- 已拒绝状态：恢复和删除 -->
            <template v-if="course.state === 'REJECTED'">
              <!-- 恢复按钮 -->
              <v-btn
                variant="flat"
                color="orange-lighten-4"
              ...
              <div class="rejection-reason">
                <div class="text-body-2 text-red-darken-1">
                  <div class="d-flex align-center">
                    <v-icon icon="mdi-alert-circle" size="14" color="red-darken-1" class="mr-1"></v-icon>
                    <strong>拒绝原因：</strong>
                  </div>
        ...
        <div class="course-info">
          ...
          <div class="d-flex align-center mb-3">
            ...
            <div v-if="course.parent" class="d-flex align-center"> 
              ...
              <span class="text-caption text-grey-darken-1 ps-4">
                父课程：<a :href="`/read?courseId=${course.parent.id}`" target="_blank"> {{ course.parent.name }} </a>
              </span>
            </div>
          </div>
          ...
          <div class="d-flex align-center mb-3">
            ...
            <v-chip
              ...
              size="small"
            >
              <v-icon icon="mdi-folder-multiple" color="green-darken-2" size="14" class="mr-2"></v-icon>
              <span class="text-green-darken-2">{{ getSubCategoryName(course.mainCategory, course.subCategory) }}</span>
            </v-chip>
            <v-chip
            ...
            <v-row no-gutters class="mb-3">
              <v-col cols="12">
                <div class="d-flex align-start">
                  <v-icon icon="mdi-text-long" size="16" color="grey-darken-1" class="mr-2 mt-1"></v-icon>
...
// Emits
const emit = defineEmits(['edit', 'approve', 'reject', 'delete', 'restore'])

// 状态配置
const getStateConfig = (state) => {
  const configs = {
    'SUBMITTED': { text: '待审核', color: 'warning', icon: 'mdi-clock-outline' },
    'APPROVED': { text: '已通过', color: 'success', icon: 'mdi-check-circle' },
    'REJECTED': { text: '已拒绝', color: 'error', icon: 'mdi-close-circle' }
  }
