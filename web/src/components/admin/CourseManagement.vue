    ...
    <v-window v-model="activeTab">
      ...
      <v-window-item :value="0">
        ...
        <div v-else-if="courseList.length === 0" class="text-center py-12">
          ...
          <div
            v-if="
              getCurrentState() === COURSE_STATE.APPROVED &&
...
<script setup>
  ...
  const stateOptions = [
    {
      value: COURSE_STATE.SUBMITTED,
      text: '待审核',
      color: 'orange-lighten-4',
      icon: 'mdi-clock-outline',
    },
    {
      value: COURSE_STATE.APPROVED,
      text: '已批准',
      color: 'green-lighten-4',
      icon: 'mdi-check-circle-outline',
    },
    {
      value: COURSE_STATE.REJECTED,
      text: '已拒绝',
      color: 'red-lighten-4',
  ...
  const loadCourses = async (isLoadMore = false) => {
    try {
      ...
      if (
        currentState === COURSE_STATE.APPROVED &&
  ...
  const confirmReject = async () => {
    ...
    try {
      ...
      if (response.code === 200) {
        showSnackbar(
          selectedCourse.value.state === COURSE_STATE.APPROVED ? '已撤销通过' : '已拒绝申请'
        )
