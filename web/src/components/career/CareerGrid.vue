    ...
    <div v-if="searchText.trim() || (activeFirstLvl !== -1 && activeSecondLvl === -1)">
      <h4 class="text-subtitle-1 font-weight-bold text-grey-darken-3 mb-3">
        <span v-if="searchText.trim()">
          {{ t('careerCenter.listing.searchResults', { query: searchText, count: displayedCareers.length }) }}
        </span>
        <span v-else-if="activeFirstLvl !== -1">
          {{ getCategoryName(activeFirstLvl) }} - {{ t('careerCenter.listing.categoryJobs', { count: currentCareers.length }) }}
        </span>
      </h4>
      ...
      <div v-if="!loading && displayedCareers.length === 0" class="text-center py-12">
        ...
        <h3 class="text-h6 text-grey-darken-1 mb-2">
          {{ searchText.trim() ? t('careerCenter.empty.noSearchResults') : t('careerCenter.empty.noRelatedJobs') }}
        </h3>
        <p class="text-body-2 text-grey mb-4">
          {{ searchText.trim() ? t('careerCenter.empty.noSearchResultsDesc') : t('careerCenter.empty.noRelatedJobsDesc') }}
        </p>
      </div>
