<script setup>
  ...
  // Emits
  const emit = defineEmits(['update:selectedNavTab', 'search', 'openCreateDialog'])
    ...
    <v-row justify="start" class="mb-4">
      <v-col cols="12">
        <div class="d-flex align-center justify-space-between mb-3">
          ...
          <div class="d-flex align-center">
            <v-btn-toggle
              :model-value="selectedNavTab"
              variant="text"
              color="primary"
              class="nav-toggle"
              @update:model-value="selectedNavTabModel = $event"
            >
              ...
              <v-btn
                value="courses"
                class="nav-btn"
                :class="{ 'nav-btn-active': selectedNavTab === 'courses' }"
              >
                {{ t('course.center') }}
              </v-btn>
            </v-btn-toggle>
