      ...
      <v-card-text class="pa-4">
        ...
        <div class="d-flex start align-baseline mb-2">
          <v-btn 
            variant="text" 
            :ripple="false" 
            class="ma-0 pa-1 pb-1 me-3 text-h5 font-weight-bold"
        ...
        <v-expand-transition>
          <div v-if="isExpanded" class="mt-3">
            ...
            <!-- 课程列表 -->
            <v-expand-transition>
              <v-tabs-window :model-value="selectedSubCategory" v-if="hasSelectedSubCategory" class="mt-3">
                <v-tabs-window-item v-for="(subCategory, subIndex) in category.list" :key="subIndex">
                  <CourseItemsList
                    :courses="courses"
                    :loading="loading"
