...
const loadSubscription = async () => {
  ...
  try {
    let response = '';
    response = await subscriptionServiceV1.getUserSubscriptions(targetUserId.value);
...
<template>
  <div>
    <div class="mb-5 py-3 rounded text-grey d-flex align-center">
      ...
      <span class="text-body-2">{{ canEdit ? '将鼠标放置在课程上可以查看简介，操作删除，拖动图标可以切换顺序' : '查看用户关注的课程' }}</span>
    </div>
    ...
    <v-item-group class="mt-3 mb-5" column>
      <v-item>
        ...
        <div v-if="subscriptions.length === 0" class="text-center py-4">
          ...
          <p class="text-caption text-grey">调试：canEdit={{ canEdit }}, 数据长度={{ subscriptions.length }}</p>
        </div>
      </v-item>
    </v-item-group>
