  ...
  if (langCode !== currentLocale.value) {
    ...
    if (import.meta.env.PROD) {
      ...
      if (langCode === 'zh' && !currentHost.includes('zh.')) {
        window.location.href = window.location.href.replace(currentHost, `zh.${currentHost.replace('en.', '')}`)
      } else if (langCode === 'en' && !currentHost.includes('en.')) {
        window.location.href = window.location.href.replace(currentHost, `en.${currentHost.replace('zh.', '')}`)
      }
    }
  }
}
