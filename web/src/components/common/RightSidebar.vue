    ...
    <v-card v-if="actualEnabledModules.includes('platform')" flat color="grey-lighten-5" rounded="xl" class="mb-6">
      <v-card-text class="pa-4">
        <div class="d-flex align-center justify-space-between mb-3">
          ...
          <!-- 刷新按钮 -->
          <v-btn v-if="!platformStats || statsError" variant="text" size="small" color="primary" :loading="statsLoading"
            @click="refreshStats" :disabled="statsLoading">
            ...
            <v-col cols="6" class="pa-0-5">
              <div class="data-item pa-1 rounded bg-white text-center">
                <div class="d-flex flex-column align-center justify-center" style="min-height: 40px;">
                  <div class="skeleton-line skeleton-shimmer mb-1"
                    style="width: 60%; height: 12px; border-radius: 6px;"></div>
                  <div class="skeleton-line skeleton-shimmer" style="width: 60%; height: 12px; border-radius: 6px;">
                  </div>
                </div>
              </div>
            </v-col>
        ...
        <div v-else>
          ...
          <v-row dense>
            <v-col cols="6" class="pa-0-5">
              <div class="data-item pa-1 rounded bg-white text-center data-item-clickable"
                @click="router.push('/course/list')">
                <div class="position-relative">
                  <v-icon icon="mdi-open-in-new" size="12" color="primary" class="jump-icon"
                  ...
                  <div class="text-caption text-grey-darken-3" style="font-size: 10px;">{{ t('rightSidebar.courseTotal')
                    }}</div>
                </div>
              </div>
            </v-col>
            <v-col cols="6" class="pa-0-5">
              <div class="data-item pa-1 rounded bg-white text-center data-item-clickable"
                ...
                <div class="position-relative">
                  <v-icon icon="mdi-open-in-new" size="12" color="teal" class="jump-icon"
                    @click.stop="router.push('/career')">
                  </v-icon>
                  <div class="text-subtitle-1 font-weight-bold text-teal mb-0">
                    {{ platformStats?.careerPathCount || '156' }}
                  </div>
                  <div class="text-caption text-grey-darken-3" style="font-size: 10px;">{{ t('rightSidebar.careerPaths')
                    }}</div>
                </div>
              </div>
            </v-col>
            <v-col cols="4" class="pa-0-5">
              <div class="data-item pa-1 rounded bg-white text-center data-item-static">
                ...
                <div class="text-caption text-grey-darken-3" style="font-size: 10px;">{{
                  t('rightSidebar.learningRoadmaps') }}</div>
              </div>
            </v-col>
            <v-col cols="4" class="pa-0-5">
              <div class="data-item pa-1 rounded bg-white text-center data-item-static">
                ...
                <div class="text-caption text-grey-darken-3" style="font-size: 10px;">{{
                  t('rightSidebar.knowledgeNodes') }}</div>
              </div>
            </v-col>
            <v-col cols="4" class="pa-0-5">
              <div class="data-item pa-1 rounded bg-white text-center data-item-static">
                ...
                <div class="text-caption text-grey-darken-3" style="font-size: 10px;">{{ t('rightSidebar.articleCount')
                  }}</div>
              </div>
            </v-col>
          </v-row>
      ...
      <v-card-text class="pa-4">
        ...
        <div v-for="(profession, index) in hotProfessions" :key="profession.id"
          ...
          <div class="flex-grow-1">
            ...
            <div class="text-caption text-grey-darken-2">{{ t('rightSidebar.peopleStudying', { count:
              (profession.learnerCount || 0).toLocaleString() }) }}</div>
          </div>
        </div>
      </v-card-text>
    </v-card>
      ...
      <v-card-text class="pa-4">
        ...
        <div v-for="(course, index) in hotCourses" :key="course.id"
          ...
          <div class="flex-grow-1">
            ...
            <div class="text-caption text-grey-darken-2">{{ t('rightSidebar.peopleBookmarked', { count:
              ((course.learnerCount || 0) + (course.subscriptionCount || 0)).toLocaleString() }) }}</div>
          </div>
        </div>
      </v-card-text>
    </v-card>
