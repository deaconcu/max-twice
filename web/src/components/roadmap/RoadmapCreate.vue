<script setup>
  ...
  // 创建根节点的通用函数
  const createRootNode = (position = { x: 400, y: 100 }) => {
    return {
      id: '0',
      type: 'default',
      data: {
  ...
  const emit = defineEmits(['update:modelValue', 'close', 'save', 'roadmap-saved'])
  ...
  // 搜索相关方法
  const loadAvailableCourses = async (searchName = '') => {
    try {
