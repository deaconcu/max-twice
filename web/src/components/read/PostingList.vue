<script setup>
  ...
  const emit = defineEmits(['load-data', 'node-completed', 'start-learning', 'loadData'])
   ...
   */
  const initializePostViewTracking = () => {
    // 等待DOM渲染完成后开始跟踪
    nextTick(() => {
      // 自动扫描并开始跟踪页面上的posts
  ...
  const loadData = (parts) => {
    emit('loadData', parts)
  }

  const updateLastPostingId = () => {
...
<template>
  ...
  <template v-else-if="tab === 'comment'">
    <v-row class="pa-0 ma-0 my-8">
      <CommentArea :object="data.node" :type="1"></CommentArea>
    </v-row>
  </template>
  ...
  <template v-else>
    ...
    <v-row class="pa-0 ma-0 my-5">
      <CommentArea :object="currPosting" :type="0"></CommentArea>
    </v-row>
  </template>
