<script setup>
  ...
  const load = async ({ done }) => {
    try {
      ...
      // Handle error without using error parameter
    }
  }
...
<template>
  ...
  <v-infinite-scroll :key="scrollKey" :items="comments" @load="load" class="w-100">
    <div
      ...
      :key="comment.id"
      :ref="comment.id === parseInt(route.query.commentId) ? setTargetRef : null"
    >
      <v-row
      ...
      >
        ...
        <div class="comment-content">
          ...
          <SubcommentArea
            ...
            :offset-id="
              comment.id === parseInt(route.query.commentId) && 'subCommentId' in route.query
                ? route.query.subCommentId
                : 0
            "
          ></SubcommentArea>
        </div>
