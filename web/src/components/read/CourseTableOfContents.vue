...
const emit = defineEmits(['update:currContentsIndex', 'update:openContentsList', 'update:configContents', 'loadData']);
...
const localCurrContentsIndex = computed({
  get: () => props.currContentsIndex,
  set: (value) => emit('update:currContentsIndex', value)
});

const localOpenContentsList = computed({
  get: () => props.openContentsList,
  set: (value) => emit('update:openContentsList', value)
});

const localConfigContents = computed({
  get: () => props.configContents,
  set: (value) => emit('update:configContents', value)
});
...
<template>
  <div class="sticky-left hidden-scrollbar">
    ...
    <v-tabs-window v-model="localCurrContentsIndex" class="pt-4">
      <v-tabs-window-item v-for="(item, index) in data.toc" :value="index" :key="index">
        <TreeNode 
          ref="treeNodeRefs"
