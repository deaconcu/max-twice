<script setup>
  ...
  import { POST_TYPE, VOTE_TYPE } from '@/constants/statusConstants'
...
<template>
  ...
  <div :data-post-id="posting.id" class="posting-container">
    ...
    <v-row class="ma-0 pt-5 d-flex justify-space-between" align="center">
      ...
      <template v-if="posting.type == POST_TYPE.CONTENTS">
        <v-btn
          :variant="posting.voteType === VOTE_TYPE.HELPFUL ? 'flat' : 'flat'"
          rounded="lg"
          density="comfortable"
          :color="posting.voteType === VOTE_TYPE.HELPFUL ? 'green-lighten-4' : 'grey-lighten-3'"
          class="px-4"
          @click="upvote(posting, VOTE_TYPE.HELPFUL)"
        >
          <v-icon
            v-if="posting.voteType === VOTE_TYPE.HELPFUL"
            icon="mdi-check"
            size="16"
            class="mr-2"
          ...
          <v-icon
            ...
            size="16"
            class="mr-2"
            :color="posting.voteType === VOTE_TYPE.HELPFUL ? 'green-darken-2' : 'grey-darken-2'"
          ></v-icon>
          <span
            :class="
              posting.voteType === VOTE_TYPE.HELPFUL
                ? 'font-weight-medium text-green-darken-2'
                : 'font-weight-medium text-grey-darken-2'
            "
            >{{ t('posting.agree') }} {{ posting.helpful }}</span
          >
        </v-btn>
      </template>
      ...
      <template v-else>
        <div class="d-flex">
          <v-btn
            :variant="posting.voteType === VOTE_TYPE.TWICE ? 'flat' : 'flat'"
            rounded="lg"
            density="comfortable"
            :color="posting.voteType === VOTE_TYPE.TWICE ? 'teal-lighten-4' : 'grey-lighten-3'"
            class="px-3"
            @click="upvote(posting, VOTE_TYPE.TWICE)"
          >
            <v-icon
              v-if="posting.voteType === VOTE_TYPE.TWICE"
              icon="mdi-check"
              size="14"
              class="mr-2"
              color="teal-darken-2"
            ...
            <v-icon
              ...
              size="14"
              class="mr-2"
              :color="posting.voteType === VOTE_TYPE.TWICE ? 'teal-darken-2' : 'grey-darken-2'"
            ></v-icon>
            <span
              :class="
                posting.voteType === VOTE_TYPE.TWICE
          ...
          <v-btn
            ...
            rounded="lg"
            density="comfortable"
            :color="posting.voteType === VOTE_TYPE.HELPFUL ? 'brown-lighten-4' : 'grey-lighten-3'"
            class="px-3 ms-3"
            @click="upvote(posting, VOTE_TYPE.HELPFUL)"
          >
            <v-icon
            ...
            <v-icon
              ...
              size="14"
              class="mr-2"
              :color="posting.voteType === VOTE_TYPE.HELPFUL ? 'brown-darken-2' : 'grey-darken-2'"
            ></v-icon>
            <span
              :class="
                posting.voteType === VOTE_TYPE.HELPFUL
                  ...
                  : 'font-weight-medium text-grey-darken-2'
              "
              >{{ t('posting.helpful') }} {{ posting.helpful }}</span
            >
          </v-btn>
        </div>
      </template>
