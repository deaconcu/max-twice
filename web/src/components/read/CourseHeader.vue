...
const emit = defineEmits(['start-learning', 'subscribe-course', 'show-details', 'apply-course', 'toggle-display', 'subcourse-created']);
    ...
    <div class="d-flex align-center">
      <div class="course-info">
        ...
        <div class="d-flex align-center mb-4">
          ...
          >
            <span class="font-weight-black text-body-1">{{ t('read.course.subCourse', { name: currentCourse.name }) }}</span>
          </v-chip>
        </div>
      ...
      <div class="course-actions">
        ...
        <div class="learning-status-container clickable" :class="{ 'learning-active': isLearning }" @click="startCourse">
          ...
          <div class="d-flex justify-space-between align-center" :class="{ 'mb-1': isLearning }">
            <div class="d-flex align-center">
              <v-icon 
                :icon="!isLearning ? 'mdi-play-circle-outline' : ((currentCourse?.progress || 0) >= 10000 ? 'mdi-check-circle' : 'mdi-play-circle')"
                size="16" 
                class="mr-1"
                :color="!isLearning ? 'grey-darken-2' : ((currentCourse?.progress || 0) >= 10000 ? 'success' : 'primary')"
              ></v-icon>
              <span class="text-body-2 font-weight-bold" :class="!isLearning ? 'text-grey-darken-3' : ((currentCourse?.progress || 0) >= 10000 ? 'text-success' : 'text-primary')">
                {{ !isLearning ? t('read.course.startLearning') : ((currentCourse?.progress || 0) >= 10000 ? t('read.course.learningCompleted') : t('read.course.learningInProgress')) }}
              </span>
            </div>
            <!-- 只有在学习状态下才显示进度百分比 -->
            <span v-if="isLearning" class="text-caption font-weight-bold">{{ parseFloat(((currentCourse?.progress || 0) / 100).toFixed(2)) }}%</span>
          </div>
